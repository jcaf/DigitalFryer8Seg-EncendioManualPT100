...
#define TERMOPILA_VOLTAGE 0.2f//0.6V
	...
	while (1)
	{
		...
		if (temperature_job())
		{
			...
			if (TCtemperature == MAX6675_THERMOCOUPLED_OPEN)
			{
				...
				error.table[ERROR_IDX_THERMOCOUPLE] = 1;//ERROR POR NO CONECTADO
			...
			else
			{
				...
				{
					error.table[ERROR_IDX_THERMOCOUPLE] = 2;//ERROR POR ESTAR FUERA DEL RANGO DE MEDICION
					main_schedule.bf.status_thermocuple = STATUS_THERMOCOUPLE_BAD;
				}
				else
				{
					error.table[ERROR_IDX_THERMOCOUPLE] = 0;
...
//+++++++++++++++++++++++++
		...
		uint16_t ADC_temp = ( ((uint16_t)ADCH)<<8) + ADCL;
		//if ( ADC_read(ADC_CH_0) >= TERMOPILA_ADC_VALUE)
		if ( ADC_temp >= (uint16_t)TERMOPILA_ADC_VALUE)
		{
			main_schedule.bf.status_thermopile = STATUS_THERMOPILE_OK;
			error.table[ERROR_IDX_THERMOPILE] = 0;//NO ERROR
		}
		else
		{
			main_schedule.bf.status_thermopile = STATUS_THERMOPILE_BAD;
			error.table[ERROR_IDX_THERMOPILE] = 1;//ERROR
		}



		//El highlimit que normalmente este a pullup con el contacto normalmente abierto, y se activara cuando cierra el contacto,
		if (PinRead(PORTRxHIGHLIMIT, PINxHIGHLIMIT) == 0 )
		{
			main_schedule.bf.status_highlimit = STATUS_HIGHLIMIT_BAD;
			error.table[ERROR_IDX_HIGHLIMIT] = 1;//ERROR
		}
		else
		{
			main_schedule.bf.status_highlimit = STATUS_HIGHLIMIT_OK;
			error.table[ERROR_IDX_HIGHLIMIT] = 0;//NO ERROR
		}
